...
def extract_r3(path):
    ...
    for csv_file_path in glob(os.path.join(path, '*[!test]', '*[!x].csv')):
        ...
        df['r3'] = exp_df['r3']
        df['k_opt'] = exp_df['r3t_opt']
        df['k_err'] =  exp_df['r3t_err']
        df['r30_opt'] = exp_df['r30_opt']
        df['r30_err'] = exp_df['r30_err']
...
def plot_r3(dfs, output_plot_dir):
    ...
    for df in dfs:
        ...
        df['r30_opt'] = df['r30_opt'] * space_scale**3 * 1e18
        df['k_opt'] = df['k_opt'] * space_scale**3 * 1e18
        if df['sucrose_conc'].iloc[0] == 10:
            if df['IBP'].iloc[0] == 'X':
                axs[0][0].scatter(df['times'], df['r3'], label=f"{df['IBP_conc'].iloc[0]} $\mu$M", s=0.5)
                axs[0][0].plot(df['times'], linear_func(df['times'], df['k_opt'], df['r30_opt']))
                axs[1][0].scatter(df['times'], df['r3'], label=f"{df['IBP_conc'].iloc[0]} $\mu$M", s=0.5)
                axs[1][0].plot(df['times'], linear_func(df['times'], df['k_opt'], df['r30_opt']))
            ...
            if df['IBP'].iloc[0] == 'X':
                ...
                axs[0][1].plot(df['times'], linear_func(df['times'], df['k_opt'], df['r30_opt']))
                axs[1][1].scatter(df['times'], df['r3'], label=f"{df['IBP_conc'].iloc[0]} $\mu$M", s=0.5)
                axs[1][1].plot(df['times'], linear_func(df['times'], df['k_opt'], df['r30_opt']))
            elif df['IBP'].iloc[0] == 'WT':
                axs[0][1].scatter(df['times'], df['r3'], label=f"{df['IBP_conc'].iloc[0]} $\mu$M", s=0.5)
                axs[0][1].plot(df['times'], linear_func(df['times'], df['k_opt'], df['r30_opt']))
            elif df['IBP'].iloc[0] == 'T18N':
            ...
            if df['IBP'].iloc[0] == 'X':
                ...
                axs[0][2].plot(df['times'], linear_func(df['times'], df['k_opt'], df['r30_opt']))
                axs[1][2].scatter(df['times'], df['r3'], label=f"{df['IBP_conc'].iloc[0]} $\mu$M", s=0.5)
                axs[1][2].plot(df['times'], linear_func(df['times'], df['k_opt'], df['r30_opt']))
            elif df['IBP'].iloc[0] == 'WT':
                axs[0][2].scatter(df['times'], df['r3'], label=f"{df['IBP_conc'].iloc[0]} $\mu$M", s=0.5)
                axs[0][2].plot(df['times'], linear_func(df['times'], df['k_opt'], df['r30_opt']))
            elif df['IBP'].iloc[0] == 'T18N':
    ...
    # Place axis labels
    axs[0][0].set_ylabel(r"$\langle r \rangle^3$ [$\mu$m$^3$]")
    axs[1][0].set_ylabel(r"$\langle r \rangle^3$ [$\mu$m$^3$]")
