...
if __name__ == "__main__":
    ...
    for i in range(1,len(frame_list)):
        ...
        for target_index, target_crys in enumerate(list(crystal_tracking_list)): # loop through a copy because we need to potentially add something to the original list
                ...
                for crys in frame_list[i].crystalobjects:
                    if (crys.center_arr == clostest_coord).all():
                        if crys.area*(1-AREA_PCT) <= target_crys.areas[len(target_crys.areas) -1] <= crys.area*(1+AREA_PCT):
                            ...
                            if crys.hierarchy[3] != -1:
                                print(f'Shite! Im tracking the child as a cunt')
            ...
            if found_matching_crystal == False:
                ...
                for area_combination in combinations(c_crystal_areas_list, 2):
                    ...
                    if target_area * (1 - 0.5*AREA_PCT) <= sum(area_combination) <= target_area * (1 + 0.5*AREA_PCT):
                        # print("------open split prints------")
                        # print(f'Found two crystals which area sum matches the current crystal at {target_center}')
                        ...
                        if EUCL_distance(target_center, split_products_COM) < MAX_CENTER_DISTANCE:
    ...
    for i,b in enumerate(crystal_tracking_list):
        ...
        if b.c_count > MIN_PLOT_FRAMES:
            ...
            fig_ax1.invert_yaxis()
            fig_ax1.title.set_fontsize(12)
            fig.suptitle(t=f'#{b.count_num}; FU{b.c_count}/{file_count}; start frame {b.frames_used[0]}', fontsize=12, va='top')
