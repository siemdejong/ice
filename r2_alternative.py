...
def plot_r2(dfs, output_plot_dir):
    ...
    for df in dfs:
        df['r2'] = df['r2'] * space_scale**2 * 1e12
        if df['sucrose_conc'].iloc[0] == 10:
            if df['IBP'].iloc[0] == 'X':
                axs[0][0].scatter(df['times'], df['r2'], label=f"{df['IBP_conc'].iloc[0]} $\mu$M", s=0.5)
                # axs[0][0].plot(df['times'], linear_func(df['times'], df['At_opt'], df['A0_opt']))
                axs[1][0].scatter(df['times'], df['r2'], label=f"{df['IBP_conc'].iloc[0]} $\mu$M", s=0.5)
            ...
            if df['IBP'].iloc[0] == 'X':
                axs[0][1].scatter(df['times'], df['r2'], label=f"{df['IBP_conc'].iloc[0]} $\mu$M", s=0.5)
                # axs[0][1].plot(df['times'], linear_func(df['times'], df['At_opt'], df['A0_opt']))
                axs[1][1].scatter(df['times'], df['r2'], label=f"{df['IBP_conc'].iloc[0]} $\mu$M", s=0.5)
            ...
            if df['IBP'].iloc[0] == 'X':
                axs[0][2].scatter(df['times'], df['r2'], label=f"{df['IBP_conc'].iloc[0]} $\mu$M", s=0.5)
                # axs[0][2].plot(df['times'], linear_func(df['times'], df['At_opt'], df['A0_opt']))
                axs[1][2].scatter(df['times'], df['r2'], label=f"{df['IBP_conc'].iloc[0]} $\mu$M", s=0.5)
    ...
    # Place axis labels
    axs[0][0].set_ylabel("r2 [$\mu$m$^3$]")
    axs[1][0].set_ylabel("r2 [$\mu$m$^3$]")
    ...
    fig.savefig(os.path.join(output_plot_dir, 'r2 A div l'), bbox_inches='tight')
    plt.show()
...
if __name__ == '__main__':
    ...
    df = extract_r2(INPUT_FOLDER_NAME)
    plot_r2(df, OUTPUT_FOLDER_NAME)